...
public class EnumGenerator : IIncrementalGenerator
{
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterPostInitializationOutput(ctx => ctx.AddSource(
            ...
            SourceText.From(EnumSourceGenerationHelper.AttributeSource, Encoding.UTF8)));
    ...
    static EnumDeclarationSyntax? GetSemanticTargetForGeneration(GeneratorSyntaxContext context)
    {
        ...
        foreach (AttributeListSyntax attributeListSyntax in enumDeclarationSyntax.AttributeLists)
        {
            foreach (AttributeSyntax attributeSyntax in attributeListSyntax.Attributes)
            {
                ...
                if (fullName == EnumSourceGenerationHelper.AttributeName)
                {
                    // return the enum
    ...
    static void Execute(Compilation compilation, ImmutableArray<EnumDeclarationSyntax> enums, SourceProductionContext context)
    {
        ...
        if (enumsToGenerate.Count > 0)
        {
            ...
            string result = EnumSourceGenerationHelper.GenerateExtensionClass(enumsToGenerate);
